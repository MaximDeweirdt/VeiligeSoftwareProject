...
//				(byte) (Integer.parseInt("" + pinInput.charAt(3))), };
	...
	public void keyAgreementLCPAndCard() throws Exception {
		...
		gui.addText("symmetric key with LCP = " + new BigInteger(1, symmetricKey).toString(16));
		gui.addText("");
	}
	...
	public void requestCertificate() throws Exception {
		...
		bb.put(r.getData());
		cardCert = bb.array();
		gui.addText("certificaat = " + new BigInteger(1, cardCert).toString(16));
		gui.addText("versturen van het cardCert");
	...
	private void checkCorrectCertificate(byte[] input) throws Exception {
		...
		gui.addText(new String(response) + " certificate");
		if (!response.equals("accepted")) {
			gui.addText("Certificaat niet geaccepteerd");
			correctCardCert = false;
		} else {
			gui.addText("Certificaat geaccepteerd");
